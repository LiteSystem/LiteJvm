...
ClassFile* ClassLoader::readClassFile(FILE *fp) {    
    ...
    // 52 1.8
    if (isVersionValid(classFile, 52) == false) {
...
StackMapTable_attribute ClassLoader:: getAttributeStackMapTable( FILE * fp){
	    StackMapTable_attribute result;
	//
	    result.number_of_entries = readU2(fp);
	   //   cout<<" StackMapTable_attribute "<< result.number_of_entries <<endl;

	    for (u2 i = 0; i < result.number_of_entries; i++) {
         	u1 type= readU1(fp);

	      if(  type >=0 && type <=63){
	    	 //  cout <<" frame_type: SAME " ;
	     }else     if (type >=64 && type <=127){
	    	//    cout <<"frame_type: SAME_LOCALS_1_STACK_ITEM " ;
	    	    verificationTypeInfo(fp) ;
	     } else    if (type ==247){
	    		u2 offet_delta = readU2(fp);
	    	//	 cout << "frame_type: SAME_LOCALS_1_STACK_ITEM_EXTENED. offet_delta"<<offet_delta  <<endl;
	    		 verificationTypeInfo(fp) ;
	     }else if (type >=248 && type <=250) {
	    		u2 offet_delta = readU2(fp);
	    	// cout << "frame_type: CHOP, offet_delta: "<< offet_delta ;

	     }else  if (type == 251){
	    		u2 offet_delta = readU2(fp);
	    	//    cout <<"frame_type: SAME_FRAME_EXTENED, offet_delta: "<< offet_delta <<endl;

	     } else if (type >=252 && type <=254){
	    	 u2 offet_delta = readU2(fp);
	     //   cout <<"frame_type: APPEND, offet_delta: "<< offet_delta  <<endl ;
	        for(int j=0;j<type-251;j++)
	    	 ...
	    	 u2 offet_delta = readU2(fp);
	    	 u2 number_of_locals = readU2(fp);

	    //	 cout << "frame_type: FULL_FRAME, offet_delta: "<< offet_delta<< " number_of_locals: "<<number_of_locals <<endl;
	    	 for(int j=0;j<number_of_locals;j++)
	    	            {
	    	                verificationTypeInfo(  fp );
	    	            }
	    	  int number_of_stack_items= readU2(fp);
	     ...
	     cerr <<"unknow  StackMapTable type " <<type<<endl;
	    	   }
...
void  ClassLoader::verificationTypeInfo(FILE *fp ) {
	 u1 tag   = readU1(fp);
	//     cout << "VerificationTypeInfo. tag: "<< tag<<endl ;
	    switch (tag)
	    {
	        case 0:
	        case 1:
	        ...
	        case 8:
	                 readU2(fp );
	   //       cout << "offset or cpool_index: "<< cpool_index  <<endl  ;
	          break;
	        default :
	          cout << "VerificationTypeInfo. switch default branch! tag: "<< tag <<endl;
	    }
}
...
attribute_info ClassLoader::getAttributeInfo(FILE *fp, ClassFile *classFile) {
    ...
    } else if(Utils::compareUtf8WithString(name, "StackMapTable")) {
        result.info.stackMapTable_info = getAttributeStackMapTable(fp);

    } else {
        //cerr << "Arquivo .class possui uma um atributo invalido." << endl;
        cerr << "  .class   unkown attribute,  result.attribute_name_index :"<<( result.attribute_name_index  ) <<endl;
